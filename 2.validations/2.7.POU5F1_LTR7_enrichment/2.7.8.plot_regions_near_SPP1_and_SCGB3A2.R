here::i_am("scripts/2.validations/2.7.POU5F1_LTR7_enrichment/2.7.8.plot_regions_near_SPP1_and_SCGB3A2.R")
...
SPP1_motif_score_list <- lapply(species, function(x) {
  
  scores_raw <- fread(paste0(wd, "SPP1_regions_", x, ".txt"),
                      select = c("extra_info", "cluster_id_or_motif_name", "cluster_or_motif_score", "# chrom", "strand", "genomic_start__bed", "genomic_end__bed"),
                      colClasses = c("cluster_id_or_motif_name" = "character", "# chrom" = "character", "extra_info" = "character"))
  
  setnames(scores_raw, old = c("genomic_start__bed", "genomic_end__bed", "# chrom", "extra_info"), new = c("start", "end", "seqnames", "ID"))
  
  scores_raw %>%
    as_granges() %>%
    group_by(ID) %>% 
    reduce_ranges(max_motif_score = max(cluster_or_motif_score)) %>% 
    as_tibble()
  
})
saveRDS(SPP1_motif_score_list, here(wd, "POU5F1_motif_scores_near_SPP1.rds"))
...
SCGB3A2_motif_score_list <- lapply(species, function(x) {
  
  scores_raw <- fread(paste0(wd, "SCGB3A2_regions_", x, ".txt"),
                      select = c("extra_info", "cluster_id_or_motif_name", "cluster_or_motif_score", "# chrom", "strand", "genomic_start__bed", "genomic_end__bed"),
                      colClasses = c("cluster_id_or_motif_name" = "character", "# chrom" = "character", "extra_info" = "character"))
...
saveRDS(SCGB3A2_motif_score_list, here(wd, "POU5F1_motif_scores_near_SCGB3A2.rds"))
